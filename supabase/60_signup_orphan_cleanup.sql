-- 60: Fix signup "already exists" when auth user is created but public.users is missing
-- Run in Supabase SQL Editor. Safe to re-run.
--
-- Cause: stale public.users rows with the same email (orphans without auth.users)
-- block profile insert. App then shows "already exists" even though the new auth
-- account was created successfully.

-- 1) Remove orphan profiles that no longer have a matching auth account.
delete from public.users u
where not exists (
  select 1 from auth.users a where a.id = u.id
);

-- 1b) Backfill public.users for auth accounts missing a profile row.
...
-- 2) Harden signup trigger: always clear same-email orphans before insert.
...
begin
  ...
  begin
    -- Drop stale rows with the same email (failed/partial signups).
    delete from public.users u
